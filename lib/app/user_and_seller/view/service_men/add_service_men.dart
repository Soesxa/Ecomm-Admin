    ...
    return InkWell(
      onTap: () async {
        if (_formKey.currentState?.validate() == true) {
          await UserAuthController.registerServiceEmployee(email: email.text, name: name.text, phone: phone.text, password:password.text, service_id: _serviceType?.id ?? '').then((value) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(jsonDecode(value)["message"].toString())));
            email.clear();
            name.clear();
            phone.clear();
            password.clear();
